...
#ifndef ARM_IOT_HTTPCLIENT_HPP
#define ARM_IOT_HTTPCLIENT_HPP
    ...
    namespace HTTP {
        ...
        public:
            ...
            string getSign(map<string, string> params, const string &secret) {
                ...
                return UTIL::Encrypt::md5(url + secret);
            }
            ...
            map<string, string> replenishSign(map<string, string> &params) {
                ...
                params.emplace("nonce", UTIL::Util::randomStr(16));
            ...
            nlohmann::json post(const string &url, map<string, string> params, map<string, string> headers = {},
                                int connectTimeout = 10, int timeout = 10, bool withSign = true) {
                if (withSign) {
            ...
            nlohmann::json get(const string &url, map<string, string> params = {}, map<string, string> headers = {},
                               int connectTimeout = 10, int timeout = 10, bool withSign = true) {
                if (withSign) {
                    params = this->replenishSign(params);
                }
            ...
            nlohmann::json uploadFile(const string &url, const string &filePath, map<string, string> params = {},
                                      map<string, string> headers = {}, int connectTimeout = 10, int timeout = 10,
                                      bool withSign = true) {
                if (withSign) {
                    params = this->replenishSign(params);
                }
                ...
                if (curl) {
                    ...
                    curl_formadd(&formPost, &lastptr, CURLFORM_PTRNAME, "file", CURLFORM_FILE, filePath.c_str(),
                                 CURLFORM_END);
                    if (!params.empty()) {
            ...
            bool static downloadFile(const string &remoteUrl, const string &localSavePath, int connectTimeout = 10,
                                     int timeout = 10) {
                CURL *curl;
                CURLcode res;
                curl = curl_easy_init();
                if (curl) {
                    ...
                    UTIL::Filesystem::write(localSavePath, strResponse);
            ...
            nlohmann::json parseResponse(string strResponse) {
                ...
                if (!jsonObj.contains("data") || !jsonObj.contains("code") ||
                    ...
                    throw nlohmann::json::other_error::create(400,
                                                              "JSON message missing `data`, `code`, and/or `msg` fields",
                                                              nullptr);
                }
